defmodule Elixlsx.XMLTemplates do
  ...
  defp make_data_validation({start_cell, end_cell, date: [date_validation]}) do
    {condition, date} = date_validation
    ...
    {:excelts, date} = U.to_excel_datetime(Date.to_erl(date))

    """
    <dataValidation type="date" operator="#{operator}" allowBlank="1" showErrorMessage="1" sqref="#{start_cell}:#{end_cell}">
      <formula1>#{date}</formula1>
    </dataValidation>
    """
  end
